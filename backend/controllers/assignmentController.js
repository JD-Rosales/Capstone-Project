...
const addAssignment = async (req, res) => {
  try {
    ...
    if (!title || title === "") {
      ...
      return res
        .status(400)
        .json({ message: "Assignment description is required" });
    } else if (!wordsArray || !wordsArray.length) {
      return res
        .status(400)
        .json({ message: "Please assign atleast 1 word/letter" });
    } else if (!date || !time) {
      ...
      return res
        .status(400)
        .json({ message: "Please input atleast 30 sec timer" });
    }
...
const getAssignments = async (req, res) => {
  try {
    ...
    const task = await Assignment.find({ classCode: auth.userInfo.classCode })
      .select("-password")
      .lean();
