...
const changeClassCode = async (req, res) => {
  try {
    ...
    if (!newClassCode) {
      return res
        .status(400)
        .json({ message: "Kindly input the new class code" });
    }
    ...
    const checkClassCode = await User.findOne({
    ...
    delete updatedClassCode.password; //remove the password key
    return res.status(200).json({
      user: updatedClassCode,
      token: generateToken(updatedClassCode._id),
    });
  } catch (error) {
    console.log(error);
    res.status(400).json({ message: "An error has occured" });
